...
// Use user's own photos (1.jpg to 10.jpg) for the film strip background,
// repeated to fill all three strips.
const CINEMATIC_PHOTOS = Array.from({ length: 10 }, (_, i) => `images/${i + 1}.jpg`);
...
function buildScenes() {
  ...
  lineKeys.forEach((lineIdx, sceneIdx) => {
    ...
    const tryImg = () => {
      ...
      const src = `images/${sceneIdx + 1}.${imgExts[extIdx++]}`;
      const t = new Image();
      t.onload = () => {
        if (imgResolved) return;
        imgResolved = true;
